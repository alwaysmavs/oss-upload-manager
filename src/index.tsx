...
export class UploadManager {
  ...
  private readonly onProgress?: PPTProgressListener;
  public constructor(ossClient: any, room: Room, onProgress?: PPTProgressListener) {
  ...
  public async convertFile(
    ...
    const pptURL = await this.addFile(path, rawFile, this.onProgress);
    const res: Ppt = await pptConverter.convert({
      url: pptURL,
      kind: kind,
      target: target,
      onProgressUpdated: progress => {
        if (this.onProgress) {
          this.onProgress(PPTProgressPhase.Converting, progress);
        }
      },
    });
  ...
  public async uploadImageFiles(imageFiles: File[], x: number, y: number): Promise<void> {
    ...
    await this.uploadImageFilesArray(newAcceptedFiles);
  }
  ...
  private async uploadImageFilesArray(imageFiles: NetlessImageFile[]): Promise<void> {
    if (imageFiles.length > 0) {
      ...
      await Promise.all(tasks.map(task => this.handleUploadTask(task)));
      this.room.setMemberState({
        currentApplianceName: "selector",
      });
    }
  }
  private async handleUploadTask(task: TaskType): Promise<void> {
    const fileUrl: string = await this.addFile(`${task.uuid}${task.imageFile.file.name}`, task.imageFile.file, this.onProgress);
